...
If `new Shell.CameraMonitor()` fails at enable time, the fallback behavior SHALL depend on the mode: in `auto` the ring stays on permanently with a warning; in `always` the ring stays on permanently; in `off` the ring stays off.
