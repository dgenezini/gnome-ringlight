...
GNOME Shell extension "Ring Light" (`ringlight@danielgenezini`): white border around each monitor that shrinks the work area so maximized/tiled windows stay inside it. ESM JavaScript (GJS), no build system. Automated checks: `node --test tests/extension.test.mjs` (syntax, schema keys, schema binary, metadata, color math) and `node tests/headless/run.mjs` (integration: boots a real headless GNOME Shell with two virtual monitors and drives the extension over D-Bus); CI in `.github/workflows/ci.yml` runs both on GNOME 46 (ubuntu-24.04) and GNOME 50 (ubuntu-26.04). Host runs GNOME Shell 50.3.
...
- Checks: `node --check extension.js prefs.js` for syntax; `node --test tests/extension.test.mjs` for unit + schema checks; `node tests/headless/run.mjs` for the headless GNOME Shell integration suite (CI runs all three, on GNOME 46 and 50). The headless suite needs `gnome-shell`, `dbus-run-session` and `gdbus` installed.
